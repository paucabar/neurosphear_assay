...
for (i=0; i<nWells; i++) {
	for (j=0; j<fieldsxwell;j++) {
		...
		saveAs("tif", outputCorrected+File.separator+fieldOfWiewImage+"_corrected");
		run("Run Pixel Classification Prediction", "saveonly=false projectfilename=["+project+"] inputimage=["+fieldOfWiewImage+"_corrected.tif] chosenoutputtype=Probabilities");
		images=nImages;
		for (i=1; i<=images; i++) {
			selectImage(i);
			title=getTitle();
			if (endsWith(title, "exported_data")) {
		...
		run("Stack to Images");
		selectImage("2");
		saveAs("tif", outputCorrected+File.separator+fieldOfWiewImage+"_corrected_probabilities");
		run("Close All");
	}
}
